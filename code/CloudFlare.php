...
class CloudFlare
{
    ...
    {
        if (!getenv('TRAVIS') && (!Environment::getEnv('CLOUDFLARE_AUTH_EMAIL') || !Environment::getEnv('CLOUDFLARE_AUTH_KEY'))) {
            return false;
        }
        return true;
    }
    ...
    public function getServerName()
    {
        ...
        if (!empty($_SERVER['SERVER_NAME'])) {
            $server = Convert::raw2xml($_SERVER);
            // "Fixes" #1
        ...
        $replaceWith = array('www.' => '', 'http://' => '', 'https://' => '');
    ...
    public function fetchZoneID()
    {
        if ($this->getCacheEnabled()) {
            $factory = Cache::factory(CloudFlare::class);
        ...
        if ($serverName == 'localhost') {
            Notifications::handleMessage(_t("CloudFlare.NoLocalhost", "This module does not operate under localhost." . "Please ensure your website has a resolvable DNS and access the website via the domain."), "error");
            return false;
        }
        $url = "https://api.cloudflare.com/client/v4/zones" . "?name={$serverName}" . "&status=active&page=1&per_page=20" . "&order=status&direction=desc&match=all";
        $result = $this->curlRequest($url, null, 'GET');
        $array = json_decode($result, true);
        if (!is_array($array) || !array_key_exists("result", $array) || empty($array['result'])) {
            $this->isReady(false);
            Notifications::handleMessage(_t("CloudFlare.ZoneIdNotFound", "Unable to detect a Zone ID for {server_name} under the defined CloudFlare" . " user. Please create a new zone under this account to use this module on this domain.", "", array("server_name" => $serverName)), "error");
            return false;
        }
    ...
    {
        return "Mozilla/5.0 " . "(Macintosh; Intel Mac OS X 10_11_6) " . "AppleWebKit/537.36 (KHTML, like Gecko) " . "Chrome/53.0.2785.143 Safari/537.36";
    }
    /**
    ...
    {
        if (getenv('TRAVIS')) {
            $auth = array('email' => 'person@example.com', 'key' => 'MY_SECRET_AUTH_KEY');
        } elseif (!($auth = $this->getCFCredentials())) {
            user_error("CloudFlare API credentials have not been provided.");
            exit;
        }
        $headers = array("X-Auth-Email: {$auth['email']}", "X-Auth-Key: {$auth['key']}", "Content-Type: application/json");
